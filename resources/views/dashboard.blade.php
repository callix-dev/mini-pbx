<x-app-layout>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div id="waiting-calls" class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700" x-data="waitingCallsPanel()">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
                ...
                <span class="px-3 py-1 text-sm font-medium rounded-full"
                      :class="waitingCalls.length > 0 ? 'bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400'"
                      x-text="waitingCalls.length + ' Waiting'"></span>
            </div>
            <div class="p-6">
                <!-- No waiting calls -->
                <div x-show="waitingCalls.length === 0" class="text-center py-8 text-gray-500 dark:text-gray-400">
                    <svg class="w-12 h-12 mx-auto mb-3 text-gray-300 dark:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Waiting calls table -->
                <div x-show="waitingCalls.length > 0" class="overflow-x-auto">
                    <table class="w-full">
                        <thead>
                            <tr class="text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                                <th class="pb-3">Caller</th>
                                <th class="pb-3">Group</th>
                                <th class="pb-3">Wait Time</th>
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                    <td class="py-3">
                                        <div class="flex items-center space-x-3">
                                            ...
                                            <div>
                                                <p class="font-medium text-gray-900 dark:text-white" x-text="call.caller_name || call.caller_id"></p>
                                                <p class="text-xs text-gray-500 dark:text-gray-400" x-show="call.caller_name" x-text="call.caller_id"></p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="py-3">
                                        <span class="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400" x-text="call.group_name"></span>
                                    </td>
                                    <td class="py-3">
                                    ...
                                    <td class="py-3 text-right">
                                        <div class="flex items-center justify-end space-x-2">
                                            ...
                                            <button @click="pickupCall(call.channel)" 
                                                    class="inline-flex items-center px-3 py-1.5 text-xs font-medium bg-green-500 hover:bg-green-600 text-white rounded-lg transition-colors"
                                            ...
                                            <!-- Redirect dropdown (admin/qa/manager only) -->
                                            @if(auth()->user()->hasAnyRole(['admin', 'qa', 'manager', 'super-admin']))
                                            <div x-data="{ showRedirect: false }" class="relative">
                                                <button @click="showRedirect = !showRedirect" 
                                                        class="inline-flex items-center px-3 py-1.5 text-xs font-medium bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition-colors">
                                                ...
                                                <div x-show="showRedirect" @click.away="showRedirect = false" x-cloak
                                                    ...
                                                    <template x-for="group in allGroups" :key="group.id">
                                                        <button @click="redirectCall(call.channel, group.id); showRedirect = false"
                                                                class="w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700"
                                                                x-text="group.name"
                                                                :disabled="call.group_id === group.id"></button>
                                                    </template>
                                                    <div x-show="allGroups.length === 0" class="px-4 py-2 text-sm text-gray-500 dark:text-gray-400">
                                                        No groups available
                                                    </div>
                                                </div>
                                            </div>
...
<script>
    ...
    // Waiting Calls Panel Component
    function waitingCallsPanel() {
        return {
            waitingCalls: [],
            allGroups: [],
            pickingUp: null,
            ...
            async fetchGroups() {
                try {
                    const response = await fetch('/api/waiting-calls/groups');
                    if (response.ok) {
                        const data = await response.json();
                        if (data.success) {
            ...
            async redirectCall(channel, groupId) {
                if (this.redirecting) return;
                
                this.redirecting = channel;
                
                try {
                    const response = await fetch(`/api/waiting-calls/${encodeURIComponent(channel)}/redirect`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                    ...
                    if (data.success) {
                        // Call will appear in new group after next poll
                        this.waitingCalls = this.waitingCalls.filter(c => c.channel !== channel);
                    } else {
                        alert(data.message || 'Failed to redirect call');
                    }
                } catch (e) {
                    console.error('Redirect failed:', e);
                    alert('Failed to redirect call');
                } finally {
