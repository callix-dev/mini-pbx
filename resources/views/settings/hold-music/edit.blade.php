<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">
                Edit Hold Music: {{ $holdMusic->name ?? '' }}
            </h2>
        </div>
    </x-slot>
    ...
    <div class="py-6">
        <div class="mx-auto max-w-2xl sm:px-6 lg:px-8">
            <form action="{{ route('hold-music.update', $holdMusic ?? 0) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="card">
                    ...
                    <div class="card-body space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="directory_name" id="directory_name" 
                                   value="{{ old('directory_name', $holdMusic->directory_name ?? '') }}" 
                                   class="form-input @error('directory_name') border-red-500 @enderror" required
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3" 
                                      class="form-input">{{ old('description', $holdMusic->description ?? '') }}</textarea>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-6">
                                ...
                                <div class="mt-2">
                                    <label class="inline-flex items-center">
                                        <input type="checkbox" name="is_default" value="1" 
                                               {{ old('is_default', $holdMusic->is_default ?? false) ? 'checked' : '' }}
                            ...
                            <div>
                                ...
                                <select name="is_active" id="is_active" class="form-select">
                                    <option value="1" {{ old('is_active', $holdMusic->is_active ?? true) ? 'selected' : '' }}>Active</option>
                                    <option value="0" {{ !old('is_active', $holdMusic->is_active ?? true) ? 'selected' : '' }}>Inactive</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card-footer flex items-center justify-end gap-4">
                        ...
                        <button type="submit" class="btn-primary">Update Hold Music</button>
                    </div>
                </div>
            </form>
        </div>
